...
Cursor.prototype.__publishCursor = function(subscription, callback) {
  var self = this;
  var socketSession = subscription._session;
  //assign the sessionViews object if not exits
  if(!socketSession._sessionViews) {
    socketSession._sessionViews = {};
  }

  //assign the sessionView for a collection, if not exits
  if(!socketSession._sessionViews[self._collection.name]) {
    socketSession._sessionViews[self._collection.name] = new Meteor.SmartSessionView(socketSession, self._collection.name);
  }

  this._observeChanges({
    added: function(id, doc) {
      socketSession._sessionViews[self._collection.name].added(id, doc);
    },
    changed: function(id, fields) {
      socketSession._sessionViews[self._collection.name].changed(id, fields);
    },
    removed: function(id) {
      socketSession._sessionViews[self._collection.name].removed(id);
    }
