...
Cursor.prototype._forEach = function (callback, endCallback) {
  ...
  if(Fibers.current) {
    afterNextObject = Meteor.bindEnvironment(afterNextObject, function(err) {
      Meteor.debug('error getting next object in _forEach', err.stack);
    });
  }
...
Cursor.prototype._map = function _map(mapCallback, resultCallback) {
  ...
  if(Fibers.current) {
    afterNextObject = Meteor.bindEnvironment(afterNextObject, function(err) {
      Meteor.debug('error getting next object in _map', err.stack);
    });
  }
...
Cursor.prototype._fetch = function _fetch(callback) {
  ...
  if(Fibers.current) {
    onResult = Meteor.bindEnvironment(onResult, function(err) {
      Meteor.debug('error getting results in _fetch', err.stack);
    });
  }

  cursor.toArray(onResult);
  ...
  Cursor.prototype[method] = function() {
    ...
    if(Fibers.current) {
      ...
      doApply = Meteor.bindEnvironment(doApply, function(err) {
        Meteor.debug('error in: ' + method, err.stack);
      });
    }
