...
Cursor.prototype.init = function(mongoCursor, collection, options) {
  ...
  if(typeof(this._options.sort) == 'object') {
    ...
    this._sortFields = _.uniq(_.keys(this._options.sort));
  }
};
...
Cursor.prototype._added = function _added(doc, ignoreBlock) {
  ...
  if(this._options.limit > 0 && _.keys(this._idMap).length >= this._options.limit) {
    return;
  }
  ...
  if(this.observing && !this._idMap[doc._id]) {
    ...
    //caching for sort
    if(this._sortable) {
      this._sortDocCache[doc._id] = _.pick(doc, this._sortFields);
    }
  }
};
...
Cursor.prototype._changed = function(id, fields) {
  if(this.observing && this._idMap[id]) {
    ...
    //caching for sort
    if(this._sortable) {
      this._sortDocCache[id] = _.pick(fields, this._sortFields);
    }
  }
};
