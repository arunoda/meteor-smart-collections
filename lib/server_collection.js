...
SmartCollection.prototype.find = function(selector, options) {
  ...
  if(this._collection) {
    ...
    var cursor = new Meteor.SmartCursor(mongoCursor, this, options.transform);
    return cursor;
  } else {
    var cursor = new Meteor.SmartCursor();
    this.once('ready', function() {
      var mongoCursor = self._collection.find(selector, options);
      cursor.init(mongoCursor, self, options.transform);
    });
    return cursor;
  }
};
...
SmartCollection.prototype._findOne = function(selector, options, callback) {
  ...
  function afterFound(err, result) {
    Fibers(function() {
      var transformFunc = options.transform || self._transform;
