...
SmartCollection.prototype._update = function(selector, mod, options, callback, afterPublished) {
  ...
  this._collection.update(selector, mod, options, function(err, updateCount) {
    if(err) {
      ...
      //write fence support
      if(updateCount == 0) {
        //if the updateCount == 0 means, there is nothing is updated. (happend when the document is not exists)
        //so we need to afterPublish manually, otherwise other methods are blocked
        afterPublished();
...
SmartCollection.prototype._remove = function(selector, callback, afterPublished) {
  ...
  this._collection.remove(selector, function(err, removeCount) {
    if(err) {
      if(callback) callback(err);
      afterPublished();
    } else {
