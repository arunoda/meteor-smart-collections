...
SmartCollection.prototype._insert = function _insert(document, afterPublished, callback) {
  var self = this;
  ...
  this._collection.insert(document, function(err) {
    if(err) {
      ...
      if(!Meteor.SmartMongo.oplog) {
        self.opQueue.insert(document, afterPublished);
      }
...
SmartCollection.prototype._update = function(selector, mod, options, afterPublished, callback) {
  selector = this._normalizeSelector(selector);
  var self = this;
    ...
    if(err) {
      ...
      if(!Meteor.SmartMongo.oplog) {
        if(typeof(selector._id) == 'string') {
          self.opQueue.update(selector._id, mod, afterPublished);
        } else {
...
SmartCollection.prototype._remove = function(selector, afterPublished, callback) {
  selector = this._normalizeSelector(selector);
  var self = this;
  this._collection.remove(selector, function(err) {
    if(err) {
      ...
      if(!Meteor.SmartMongo.oplog) {
        if(typeof(selector._id) == 'string') {
          self.opQueue.remove(selector._id, afterPublished);
        } else {
...
['insert', 'update', 'remove', 'findOne', '_ensureIndex', '_dropIndex'].forEach(function(writeMethod) {
  SmartCollection.prototype[writeMethod] = function() {
    ...
    if(Fibers.current) {
      console.log('method', writeMethod);
      future = new Future();
      var resolver;
