...
OpQueue.prototype._processIdUpdateQueue = function(id, modifier) {
  ...
  if(!this._idUpdateQueues[id]) {
    this._idUpdateQueues[id] = new IdUpdateQueue(afterCompleted);
...
function IdUpdateQueue(endCallback) {
  this._items = [];
  this._stopped = false;
}
...
IdUpdateQueue.prototype.start = function() {
  ...
  function start() {
    if(self._stopped) {
      ...
      var payload = self._idUpdateQueues.shift();
      if(payload) {
        self._invalidator.update(payload[0], payload[1]);
      } else {
        self._stopped = true;
        endCallback();
      }
