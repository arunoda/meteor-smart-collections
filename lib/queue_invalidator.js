...
QueueInvalidator.prototype.insert = function(doc) {
  invalidator.insert(doc);
};
...
QueueInvalidator.prototype._processNext = function(id) {
  ...
  function startProcessing(id) {
    ...
    invalidator.update(req[1], fields, afterUpdated);
  }
    ...
    while(true) {
      ...
      if(!res) {
        ...
        if(self._idsProcessing.length > 0) {
          ...
          if(res[0] == 'mu') {
            invalidator.multiUpdate(res[1], res[2], startGlobalProcessing);
          } else if(res[0] == 'mr') {
            invalidator.multiRemove(res[1], startGlobalProcessing);
          } else {
