...
Meteor.methods({
  ...
  _su_: function(collectionName, id, modifier) {
    ...
    var accessGranted = collection.validators.update.evaluate(this.userId, doc, fieldNames, modifier);
  ...
  _sr_: function(collectionName, id) {
    ...
    var selector = {_id: id};
    var doc = collection.findOne(selector);
    var accessGranted = collection.validators.remove.evaluate(this.userId, doc);
