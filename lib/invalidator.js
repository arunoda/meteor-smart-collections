...
Invalidator.prototype.invalidateMultiRemove = function(collectionName, selector) {
  var selectors = this._selectors[collectionName];
  var collection = this._collections[collectionName];
  if(selectors) {
  ...
  function pollIds(selectorInfo) {
    ...
    collection._collection.find(selectorInfo.selector, {fields: {_id: 1}}).each(function(err, doc) {
      if(err) {
        console.error('error when polling for ids', {collection: collectionName, selector: selectorInfo.selector, error: err.message});
      } else if(doc) {
