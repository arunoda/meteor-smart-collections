...
Invalidator.prototype.initiateQuery = function(selector, options) {
  ...
  for(var lc=0; lc<this._queryInfoList.length; lc++) {
    ...
    if(!EJSON.equals(options.fields, queryInfo.options.fields)) {
      continue;
    }
...
Invalidator.prototype.update = function(id, modifier, callback) {
  ...
  function afterFound(err, doc) {
    if(err) {
      ...
      self._notifyChanges(id, doc, fieldMap, afterChanged);
    } else {
...
Invalidator.prototype._notifyChanges = function(id, doc, fieldMap, callback) {
  callback = callback || function() {};
  var processingCount = 0;
  var completedCount = 0;

  var changes = _.pick(doc, _.keys(fieldMap.update));
  _.keys(fieldMap.remove).forEach(function(field) {
  ...
  this._queryInfoList.forEach(function(queryInfo) {
    ...
    if(query.idExists(id)) {
      processingCount++;
      query.changed(id, changes, afterChanged);
    }
  });
...
Invalidator.updateModifierToFields = function updateModifierToFields(modifier) {
  ...
  function pickFields(updateCommand, action) {
    if(action == 'UPDATE_ONLY') {
      ...
      for(var key in updateCommand) {
        if(hasDot(key)) {
          //when applied to a nested object, we can't consider it as remove 
          result.update[handleDot(key)] = 1;
        } else {
      ...
      for(var key in updateCommand) {
        if(hasDot(key)) {
          //we need to consider this as a normal update
          result.update[handleDot(updateCommand[key])] = 1;
          result.update[handleDot(key)] = 1;
        } else {
          result.update[handleDot(updateCommand[key])] = 1;
          result.remove[handleDot(key)] = 1;
