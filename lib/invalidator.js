...
Invalidator.prototype.update = function(id, modifier, callback) {
  ...
  function afterFound(err, doc) {
    if(err) {
      ...
      self._notifyChanges(id, doc, afterChanged);
    } else {
...
Invalidator.prototype._notifyChanges = function(id, doc, callback) {
  callback = callback || function() {};
  var processingCount = 0;
  var completedCount = 0;
  ...
  this._queryInfoList.forEach(function(queryInfo) {
    ...
    if(query.idExists(id)) {
      var changes = query._getChanges(query._docMap[doc._id], doc);
      query.changed(id, changes);
    }
  });
