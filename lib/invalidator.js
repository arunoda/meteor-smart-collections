...
Invalidator.prototype.update = function(id, modifier) {
  var self = this;
  ...
  console.log('UPDATE', id, modifier);
  ...
  function afterFound(err, doc) {
    if(err) {
      ...
      self._notifyAdded(id, doc, function() {
        console.log('REC-ADDED');
        afterChanged();
      });

      //invalidate queries for removed if this affect them
      self._notifyRemoved(id, doc, function() {
        console.log('REC-REMOVED');
        afterChanged();
      });

      //invalidate queries for update
      self._notifyChanges(id, fields, doc, function() {
        console.log('REC-CHANGES');
        afterChanged();
      });
    } else {
...
Invalidator.prototype._emitChange = function(type, id) {
  console.log('EMIT_CHANGE', arguments);
  this.emit(type + ':' + id);
};

Invalidator.prototype._onceChanged = function(type, id, callback) {
  console.log('ONCE_CHANGE', arguments);
  this.once(type + ':' + id, callback);
};
