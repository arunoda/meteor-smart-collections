...
function Query(collection, selector, options) {
  ...
  this._afterRecentSnapshotCallbacks = [];
...
Query.prototype.addObserver = function(observer, callback) {
  ...
  if(!this.snapshotInProgress) {
    ...
    if(callback) {
      this._afterRecentSnapshotCallbacks.push(callback);
    }
  }
};
...
Query.prototype.countObservers = function() {
  return this._observers.length + this._amatureObserves.length;
};
...
Query.prototype.snapshot = function(callback) {
  ...
  function fireCallbacks(err) {
    ...
    self._afterRecentSnapshotCallbacks.forEach(function(callback) {
      callback(err);
    });
    self._afterRecentSnapshotCallbacks = [];
  }
};
